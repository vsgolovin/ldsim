...
plt.plot(ld.xn, ld.vn['Ec']-ld.sol['psi'], color='k')
plt.plot(ld.xn, ld.vn['Ev']-ld.sol['psi'], color='k')
plt.plot(ld.xn, -ld.sol['phi_n'], 'b:', label=r'$\varphi_n$')
plt.plot(ld.xn, -ld.sol['phi_p'], 'r:', label=r'$\varphi_p$')
plt.legend()
plt.xlabel(r'$x$ ($\mu$m)')
plt.ylabel('$E$ (eV)')
plt.show()
